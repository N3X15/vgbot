...
class TellPlugin(IPlugin):
    def __init__(self, bot):
        ...
        # Clear messages to me.
        for name in globalConfig.get('names', []):
            self.data[name] = []
    ...
    def OnTell(self, event, args):
        ...
        if len(args) < 3:
            self.bot.privmsg(channel, 'The format is: {}, tell NICKNAME MESSAGE TO SEND'.format(self.bot.connection.get_nickname()))
            return
        
        msg = ' '.join(args[2:])
        to = args[1]
        
        if to in globalConfig.get('names', []):
            self.bot.privmsg(channel, 'I will not talk to myself, thank you very much.')
            return
    ...
    def SendMessages(self, channel, nick):
        if nick in globalConfig.get('names', []) + [self.bot.connection.get_nickname()]:
            self.data[nick] = []
            return
